...
// Query and display trabalhos trabalho type given certain data
const Trabalhos = ({ data, style }) => {
    const trabalhos = data.edges.map(trabalho => {
        ...
        let title = trabalho.node.featured_media.title ? trabalho.node.featured_media.title : trabalho.node.title

        return (
            <Link key={trabalho.node.slug} to={`/trabalhos/${trabalho.node.slug}`} className={style.Trabalhos_wrapper_singleTrabalho}>
                <figure>
                    <Img alt={alt} title={title} fixed={trabalho.node.featured_media.localFile.childImageSharp.fixed} style={{ maxHeight: '100vw' }} />
                </figure>
                <div className={style.Trabalhos_wrapper_singleTrabalho_metaBox}>
                    <h3>
                        {parser(trabalho.node.title)}
                    </h3>
